	...
	export default {
		...
		methods: {
			getData() {
				...
				uni.request({
					url: this.$serverUrl + '/api.php/video/videolist.html?page=' + (this.refreshing ? 1 : this.fetchPageNum) 
					+ '&list_rows=' + this.$listRows + '&access_token=' + access_token + '&user_token=' + login_user_data.user_token + '&cid=' + this.cid,
					success: (ret) => {

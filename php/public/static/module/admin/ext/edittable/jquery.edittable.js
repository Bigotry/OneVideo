...
(function ($, window, i) {
    ...
    $.fn.editTable = function (options) {
        ...
        function buildCell(content, type) {
            ...
            // Default
            return '<td><input type="text" value="' + content.toString().replace(/"/g, "&quot;") + '" name='+default_text_name+' /></td>';
        }
        ...
        $table.on('click', '.addrow', function () {
            ...
            var field_describe_html = $(this).parent().prev().html();
            
            if(field_describe_html.indexOf("request_data") > 0 ) { default_text_name = 'request_data[field_name][]'; }
            if(field_describe_html.indexOf("response_data") > 0 ) { default_text_name = 'response_data[field_name][]'; }
